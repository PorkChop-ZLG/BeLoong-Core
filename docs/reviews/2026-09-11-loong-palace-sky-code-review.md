...
2. **唯一的资源级问题是显存**：9 张贴图占 **180.77 MiB**，且**加载后整个会话常驻、无释放路径**。可保守降到 **≈45.4 MiB（省 74.9%）**。这是本子系统最大的单项成本，且此前从未被评估。→ **已于第九节实施第一步：现为 135.38 MiB**（月亮相关 54.52 → **9.13 MiB**）。
...
### 2.8 [次要 → 部分已解决] M5 —— 天空盒图集为 `GL_NEAREST` 且无 mipmap；mcmeta 可零显存换取 `GL_LINEAR`
...
- **状态**：月相 8 张与 `mask_moon.png` 已于 2026-09-11 **加 `.mcmeta`（`blur:true`）**；其余 6 张 skybox 图集仍为 `GL_NEAREST`。
- **影响**：不改变显存，但决定降采样的观感代价——平滑渐变层几乎无损；点/盘状特征（`stars` / `sunflare`）会块状化。
- **修复建议（剩余项）**：若将来对 `stars` / `sunflare` / `day` / `night` / `mask` / `skybox/sun` 降采样，需**同时**为它们加 mcmeta。
...
- **修复落地情况（逐项，截至 2026-09-11）**：
  ...
  | 项 | 建议 | 状态 |
  |---|---|---|
  | 2.1 I1 | 缩小资源尺寸（分档表） | **部分实施**：月亮相关 54.52 → 9.13 MiB；总量 180.77 → **135.38** MiB（第九节） |
  | 2.2 I2 | `TextureManager.preload` 预取 | **未实施**（第九节的拆分**顺带**把首帧同步解码量从 4.4 MB 图集降为 1 张小图，属缓解非解决） |
  ...
  | 2.8 M5 | 加 mcmeta 取 `GL_LINEAR` | **部分实施**：月相 8 张 + `mask_moon` 已加；其余 6 张未加 |
...
| | 原 | 现 |
|---|---|---|
| 天空贴图显存 | 180.77 MiB | **135.38 MiB** |
| 其中月亮相关 | 54.52 MiB | **9.13 MiB** |
...
- **其余 6 张天空盒图集**：未降采样、未加 mcmeta（仍为 `GL_NEAREST`）；分档与观感风险判据见 §2.7/§2.8。

...
2. `build/resources/main/data/beloong/beloong/npc_dialogue/iron_golem.json` 确实被打进产物 jar。
3. **R0 验证**：客户端资源重载后，loader 能列出并解析该文件（临时日志计数即可）。
...
| # | 步骤 | 预期 |
|---|---|---|
| 1 | 空手右键铁傀儡 | 弹出对话屏；**世界清晰不模糊**，只有底部约 28% 渐变压暗 |
| 2 | 观察第一页 | 名字金色居中、两侧各一条金色装饰线；正文白色居中；逐字出现（1 字/tick） |
| 3 | 打字中点击 | 当前页**立刻全部显示** |
| 4 | 文字显示完后 | 底部中央出现金色向下箭头（带轻微浮动） |
| 5 | 再点击 | 进入第二页；箭头消失、重新逐字 |
| 6 | 最后一页显示完后再点击 | 箭头消失，右侧弹出「离开」按钮 |
| 7 | 鼠标移到按钮上 | 底色**平滑**过渡到金色（约 200ms，非瞬变） |
| 8 | 点击「离开」 | 对话关闭，回到游戏 |
| 9 | 任意时刻按 ESC | 对话关闭 |
| 10 | 手持铁锭右键铁傀儡（示例数据为缺省 `empty_hand`） | **不触发对话**，走原版修血 |
| 10b | 把示例数据改成 `"trigger": "any"` 后 F3+T | 手持铁锭也能弹出对话（原版回血被吃掉，属预期行为） |

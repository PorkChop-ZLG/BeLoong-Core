...
| # | 裁定 |
|---|---|
| **Q1** | 移动改成**原版生物的移动方式**；默认 `MOVEMENT_SPEED` 取"**等于玩家移动速度**"的值（本档算出，见 §二）；**不要额外的奔跑状态**；当获得**额外移动速度**（迅捷效果等）时才切换为**奔跑动画** |
...
## 二、关键计算：默认 `MOVEMENT_SPEED` = **0.3162**（≈ √0.1）
...
令比值 = 1、且按原版惯例让 `speedModifier = 1.0`：

```
MOVEMENT_SPEED² = 0.1   ⇒   MOVEMENT_SPEED = √0.1 ≈ 0.31623
```

**⇒ 取 `0.3162`。** 这就是"在 1.0 档位下与走路玩家同速"的属性值。
...
| # | 改动 | 说明 |
|---|---|---|
| 1 | `createNpcAttributes()`：`MOVEMENT_SPEED` **0.1 → 0.3162** | §二；注释写清推导，并提醒"这是**原版刻度**值，不是玩家属性值" |
...
- **删除 `run` 子命令**（连同 `runTo` 的调用与 `"beloong.command.npc.run"` 语言键）⇒ 从 5 条子命令降为 4 条：
  `turn` / `walk` / `attack` / `stop`。

### 3.6 语言文件

- 删除 `beloong.command.npc.run`（中英各 1 条）；其余不动。
...
| # | 决策 | 取值 | 理由 |
|---|---|---|---|
| **D48** | 移动回到原版刻度 | `MOVEMENT_SPEED = 0.3162`（= √0.1），`walkTo` 传 `1.0` | 用户裁定 Q1。位移比 = `(档位×属性)²/0.1`（§2.1），故 √0.1 恰为"玩家走路速度"；**删除** D47 的 `1/√` 补偿 |
...
| # | 操作 | 预期 |
|---|---|---|
| 1 | `/beloong npc walk @e[…] ~ ~ ~8`，与玩家并排同向走 | **速度一致**（这是 D48 的直接验收） |
...
| **R19** | `0.3162` 是**计算值**，未实机测速 | **低**：公式两侧同构、摩擦与衰减因子相消，比值只由 `(档位×属性)²/0.1` 决定；实机第 1 项即验收。若略有偏差，改属性值即可（一行） |

...
| **D22** | 仪式粒子 | 每 **5 tick** 在自身周围**半径 4 格圆盘**内、自**上方 2 格**高处铺 **40** 颗 `irons_spellbooks:blood_ground`，靠重力滴落 | 用户要求（2026-09-20 定案，同日微调半径 3→4、每批 8→40、生成点抬到上方 2 格以营造「血向下滴落」）。两个实现要点：① **逐颗定点发送**（`sendParticles` 的 `count = 1`）—— 原版对 `count > 1` 的处理是「±offset 的**长方体**内随机」，会摊成方阵而不是圆 ② 半径取 `R × sqrt(u)` 而非 `R × u`，才保证圆盘内**面密度均匀**（后者向圆心堆积）。<br>**逐颗发送的代价**：每批 40 个包 × 28 批 = **1120 个小包**（约 160 包/秒），只发给 32 格内的玩家、每包数十字节 ⇒ 实测可忽略；这是为「圆盘而非方阵」刻意接受的代价，**不要**改成 `count > 1`。<br>**不做高度图采样**：`BloodGroundParticle` 继承 `TextureSheetParticle`，而 `Particle.hasPhysics` **默认为 `true`** 且自带 `gravity = 1.0F` ⇒ 从上方 2 格生成后会自然下坠、各自停在脚下表面，地形不平也自然。<br>粒子由服务端 `sendParticles` 发出（原版按 32 格筛玩家）⇒ **不需要把实体发给客户端，与 D2 无冲突** |
...
| 10 | 仪式期间观察宝库顶周围 | 半径 **4** 格内持续出现 `blood_ground` 血渍，**圆外四角没有**（验证 D22 的「圆盘而非方阵」）；血渍自上方 2 格**滴落**后各自落在脚下表面 | D22 |

...
| **D16** | Mixin 包分类 | 目标是**原版**类的 mixin 放 `mixin/minecraft/`；目标是第三方模组的放 `mixin/<该模组命名空间>/`；`beloong.mixins.json` 的条目写同名前缀（`"minecraft.DreadKingRitualTriggerMixin"`） | 用户裁定 —— 项目规范：**mixin 哪个模组就用哪个模组的命名空间分类**，便于一眼确认「这条在 mixin 原版」。现存 `mixin/` 根目录下仍有两条原版目标 mixin 未迁移（`PossibleBiomesFilterMixin` → `BiomeSource`、`ParameterListAccessor` → `Climate.ParameterList`），**本次不顺手重构**，避免把无关改动混进本功能 |
